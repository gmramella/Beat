...
double Utils::energy(double * amplitudesMono, unsigned int first, unsigned int size)
{
	double output = 0;
	for (int i = 0; i < size; i++) {
		output += pow(amplitudesMono[first + i], 2);
	}
	return output;
}

double Utils::energy(double * amplitudesLeft, double * amplitudesRight, unsigned int first, unsigned int size)
{
	double output = 0;
	for (int i = 0; i < size; i++) {
		output += pow(amplitudesLeft[first + i], 2) + pow(amplitudesRight[first + i], 2);
	}
	return output;
}
